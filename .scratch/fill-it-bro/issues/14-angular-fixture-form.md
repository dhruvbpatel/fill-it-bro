...
## Do exactly this
- `apps/fixture-form`: Angular 18+ standalone app, route `/deal/:dealId`, `ag-grid-angular` + `ag-grid-community`. Serve on port `FIXTURE_PORT` (env, default 4300) via root `pnpm fixture:serve`; every Playwright `webServer` and test URL in later tickets must read the same variable. CI job `fixture` builds it.
