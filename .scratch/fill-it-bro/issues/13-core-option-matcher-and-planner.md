...
## Acceptance criteria
- [ ] `matchOption('Goldman Sachs', ['Goldman Sachs Incorporated','Goldman Sachs Asset Mgmt'])` → `llm` tier called (both fuzzy candidates), `none` when no llm.
- [ ] `matchOption('goldman sachs inc.', ['Goldman Sachs Incorporated'])` → `exact` via normalisation.
- [ ] Planner snapshot test on fixture bundle with 2 party rows and `gridRowCounts.parties = 1` → exactly one `addRow`.
- [ ] `dependsOn` ordering enforced; cycle → throws `ConfigError`.
