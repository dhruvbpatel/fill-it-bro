...
## Acceptance criteria
- [ ] pytest: fake returns index 0 → response index 0; fake returns index 7 for 2 options → null.
- [ ] `/runs` then two events → 3 lines in the JSONL file with matching `runId`.
- [ ] Event with payload key `pageText` → 422.
